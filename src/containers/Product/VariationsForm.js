...
export default class VariationsForm extends Component {
  ...
  componentDidMount() {
    ...
    if (Object.keys(defaultVariation).length !== 0) {
      this.onChange({[`${defaultVariation.id}`]: defaultVariation.option.toUpperCase()})
    }
  }
  ...
  prepareFormData() {
    ...
    if (attributes !== null || attributes.length === 0) {
      ...
      for (let i = 0; i < attributes.length; i++) {
        formComponents[`${attributes[i].id}`] = Tcomb.enums.of(attributes[i].options)
...
stylesheet.select.normal.color = Color.TextDark

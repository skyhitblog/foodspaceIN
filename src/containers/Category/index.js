...
import {Timer, toast, warn, BlockTimer} from '@app/Omni';
...
class CategoryScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    const {fetchProductsByCategoryId, clearProducts, selectedCategory} = this.props;
    clearProducts();
  ...
  shouldComponentUpdate(nextProps) {
    const props = this.props;
    const changeProduct = nextProps.products.list.length != props.products.list.length;
    const changeCategory = props.selectedCategory.id !== nextProps.selectedCategory.id;
    ...
    return (
      <View style={styles.container}>
        <Animated.View style={{marginTop: marginControlBar}}>
          <ControlBar openCategoryPicker={this.openCategoryPicker}
  ...
  renderList(data) {
    ...
    const dataSource = new ListView.DataSource({rowHasChanged: (product1, product2) => product1.id !== product2.id});
  ...
  renderRow(product) {
    ...
    var isInWishList = this.props.wishListItems.find((item) => item.product.id == product.id) != undefined

...
class CategoryRow extends React.Component {
    ...
    shouldComponentUpdate(nextProps) {
        ...
        return props.isFirst !== nextProps.isFirst
            ...
            <View
                ...
                <TouchableOpacity style={styles.subContainer} onPress={onPress}>
                    <View style={styles.checkboxWrap}>
                        {isSelect
                            ? <Icon name={Icons.MaterialCommunityIcons.CheckMark} size={20} color={Color.accent}/>
                            : null
                        }
                    </View>
                    <Text style={styles.text}>{(isFirst ? '' : '--- ') + category.name}</Text>
                </TouchableOpacity>
            </View>
        );
    }

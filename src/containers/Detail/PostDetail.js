...
import {Constants, Config, Tools, Languages} from "@common";
...
export default class PostDetail extends Component {
  ...
  render() {
    ...
    const postContent = typeof post.content.rendered == 'undefined' ? '' : post.content.rendered;
    const videoUrl = post.content.rendered != null ? Tools.getLinkVideo(post.content.rendered) : '';
    ...
    const renderBanner = () => {
      return <Animated.View
        ...
        {videoUrl.length > 0
          ? <View style={styles.videoView}><Video source={videoUrl} style={styles.video} /></View>
          : <Animated.Image source={{uri: imageURL}}
                            style={[
    ...
    const renderContent = () => {
      return (
        <View style={styles.scrollViewContent}>
          <Text style={styles.detailDesc}>{Tools.getDescription(postTitle, 300) }</Text>
          <Text style={styles.author}><TimeAgo time={post.date} hideAgo={true} /> ago</Text>

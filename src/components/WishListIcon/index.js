...
class WishListIcon extends Component {
  ...
  addToWishList() {
    ...
    const isInWishList = wishList.wishListItems.find((item) => item.product.id === product.id) !== undefined
  ...
  render() {
    ...
    let clicked = (wishList.wishListItems.filter(item => item.product.id == product.id)) == "" ? false : true;

    return (
      <Button type="image"
              source={require("@images/icons/icon-love.png")}
              imageStyle={[styles.imageButton, clicked && {tintColor: Color.heartActiveWishList}]}

...
import {Text, View, Image, Platform, TouchableOpacity, Dimensions} from 'react-native';
...
export default class StepIndicator extends Component {
  ...
  render() {
    ...
    for (var i = 0; i < this.props.steps.length; i++) {
      ...
      label.push(<Text key={i}
                       style={[styles.label,
                         {width: this.labelWidth},
                         i <= this.props.currentIndex && {color: Color.Text,}]}>{item.label}</Text>)
    ...
    return (
      <View style={progressBarContainer} key={"progress-" + index}>
        <View style={progressBarBorder}>
          {index < this.props.currentIndex && <View style={progressBar}></View>}
        </View>
      </View>
    )
  }

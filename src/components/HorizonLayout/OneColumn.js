'use strict';
import React, {Component} from "react";
import {Text, TouchableOpacity, Image} from "react-native";
import css from "./style";
import {ProductPrice, WishListIcon} from "@components";
import {Images, Constants, Styles} from "@common"
import {getProductImage} from "@app/Omni"

export default class OneColumn extends Component {
  render() {
    const {imageURL, product, title, viewPost} = this.props;
    const image = typeof product.images[0] != 'undefined' ? {uri: getProductImage(product.images[0].src, Styles.width)} : Images.PlaceHolder

    return (
      <TouchableOpacity activeOpacity={0.9} style={css.panelTwoHigh} onPress={viewPost}>
        <Image defaultSource={Images.PlaceHolder} source={image} style={css.imagePanelTwoHigh} />
        <Text numberOfLines={2} style={css.nameTwo}>{title}</Text>
        <ProductPrice product={product} hideDisCount/>
        <WishListIcon product={product} />
      </TouchableOpacity>
    );
  }
}
